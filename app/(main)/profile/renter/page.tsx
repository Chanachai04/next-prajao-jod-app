...
import { RenterItem } from "@/types/renter";
...
export default function Renter() {
  ...
  const [data, setData] = useState<RenterItem[]>([]);
  ...
  const openDeleteConfirm = (renterId: RenterItem["id"]) => {
    setDeleteTarget({ type: "single", id: String(renterId) });
    setConfirmModalOpen(true);
  };

  //ฟังก์ชันลบข้อมูลผู้เช่า
  const handleDelete = async (renterId: RenterItem["id"]) => {
    const idParam = String(renterId);

    try {
      setDeletingId(idParam);
      const response = await fetch(`/api/renter?id=${idParam}`, {
        method: "DELETE",
      });
      ...
      setAlertModalDescription("ข้อมูลผู้เช่าถูกลบเรียบร้อยแล้ว");
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      // ลบทีละรายการ
      for (const renterId of selectedIds) {
        try {
          const response = await fetch(`/api/renter?id=${renterId}`, {
            method: "DELETE",
          });
          ...
          if (!response.ok || !result.success) {
            errors.push(`${renterId}: ${result.error || "ไม่สามารถลบได้"}`);
          }
        } catch (err) {
          errors.push(
            `${renterId}: ${
      ...
      if (errors.length === 0) {
        ...
        setAlertModalTitle("ลบข้อมูลสำเร็จ");
        setAlertModalDescription(
          `ลบข้อมูลสำเร็จ ${selectedIds.length} รายการ`
        );
        setAlertModalOpen(true);
      } else {
        setAlertModalType("error");
      ...
      <div className="flex flex-col lg:flex-row bg-white shadow-sm overflow-hidden min-h-[80vh]">
        ...
        <div className="flex-1 w-full overflow-y-auto max-h-[80vh]">
          <div className="hidden md:block px-4 lg:px-8">
            ...
            {/* tableแสดงข้อมูลผู้เช่าจากsupabase */}
            <table className="overflow-x-auto w-full border-collapse">
              <thead className="sticky top-0 bg-white z-10">
                <tr className="text-gray-700 text-lg sm:text-xl font-semibold border-b">
                  ...
                  <th className="py-4 sm:py-6 px-4 sm:px-6 text-center min-w-[250px]">
                    ชื่อสถานที่จอด
                  </th>
      ...
      <ConfirmModal
        ...
        description={
          deleteTarget?.type === "bulk"
            ...
            : "คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลผู้เช่านี้?"
        }
      />
